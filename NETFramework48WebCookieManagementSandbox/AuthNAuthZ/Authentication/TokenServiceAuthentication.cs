...
namespace NETFramework48WebCookieManagementSandbox.AuthNAuthZ.Authentication
{
    ...
    internal class TokenServicePrincipal : IPrincipal, ITokenServicePrincipal
    {
        ...
        public TokenServicePrincipal(UserData data, TokenServiceAuthenticationProviderConfig config, string sSOToken = null)
        {
            UserData = data;
            SessionExpirationDate = data.ExpiresUtc.UtcDateTime;
    ...
    internal class TokenServiceAuthenticationProvider : IAuthenticationProvider
    {
        ...
        private readonly AuthenticationTokenServiceAPI.AuthenticationTokenServiceAPI _APIclient;
        ...
        public TokenServiceAuthenticationProvider(TokenServiceAuthenticationProviderConfig config, string authenticationScheme)
        {
            ...
            _APIclient = new AuthenticationTokenServiceAPI.AuthenticationTokenServiceAPI(() => {
        ...
        public bool Authenticate(HttpContext httpContext)
        {
            ...
            try
            {
                ...
                if (HttpContext.Current.Request.QueryString[_Config.CacTokenParameterName] is string cacToken && !string.IsNullOrWhiteSpace(cacToken))
                {
                    ...
                    httpContext.User = new TokenServicePrincipal(cacResolveResponse.Value.UserData, _Config, cacResolveResponse.Value.Token);
                ...
                if(_Config.SessionMode == TokenServiceSessionMode.Sliding)
                {
                    ...
                    httpContext.User = new TokenServicePrincipal(response.Value, _Config, renewSlidingResponse.Value.Token);
                ...
                else if (_Config.SessionMode == TokenServiceSessionMode.Mixed)
                {
                    ...
                    {
                        var renewMixedResponse = _APIclient.RenewSession();
                        httpContext.User = new TokenServicePrincipal(response.Value, _Config, renewMixedResponse.Value.Token);
                        AddHttpCookie(renewMixedResponse.Value.Token, renewMixedResponse.Value.ExpiresUtc.UtcDateTime);
                        return true;
                    }                    
                }
                httpContext.User = new TokenServicePrincipal(response.Value, _Config, currentCookie.Value);
